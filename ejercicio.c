...
int main(int argc, char *argv[])
{
    ...
    float As = calcularArea(per1, per2, per3, per4, l1, l2, l3, l4, l5, l6, &areab);
    printf("El AS es: %f\n", As);
...
float calcularArea(float per1, float per2, float per3, float per4, float l1, float l2, float l3, float l4, float l5, float l6, float *areab)
{
    ...
    *areab = sqrt(s1 * (s1 - l1) * (s1 - l2) * (s1 - l3));
    float area1 = sqrt(s2 * (s2 - l2) * (s2 - l4) * (s2 - l6));
    float area2 = sqrt(s3 * (s3 - l3) * (s3 - l5) * (s3 - l6));
    float area3 = sqrt(s4 * (s4 - l1) * (s4 - l5) * (s4 - l4));
    float As = *areab + area1 + area2 + area3;
    return As;
}
...
{
    float bari[1][2];
    bari[0][0] = (puntos[0][0] + puntos[1][0] + puntos[2][0])/2;
    bari[0][1] = (puntos[0][1] + puntos[1][1] + puntos[2][1])/2;
    bari[0][2] = (puntos[0][2] + puntos[1][2] + puntos[2][2])/2;
    float altura = calculardistancia(puntos[0][0], puntos[0][1], puntos[0][2], puntos[3][0], puntos[3][1], puntos[3][2]);
    float volumen = (areab * altura)/3;
    return volumen;
}